...
void getPhysWfIndexes(const int& receiveLoPhysIdx, const int& receiveHiPhysIdx, ARRAY_I& data) {
        /////////////////////////////////////////////////////////////////////////////////////////////////
        //replacing 1s and 0s with actual physical waveform indexes, make sure these values are correct!
        /////////////////////////////////////////////////////////////////////////////////////////////////
        if(receiveLoPhysIdx<0 || receiveLoPhysIdx>255) {
                cerr << "Invalid receive low phys index value of " << receiveLoPhysIdx<< ", valid range is 0 to 255, exiting." << endl;
                ERROR_EXIT(TM::ABORT_FLOW);
        }
        if(receiveHiPhysIdx<0 || receiveHiPhysIdx>255) {
                cerr << "Invalid receive hi phys index value of " << receiveHiPhysIdx << ", valid range is 0 to 255, exiting." << endl;
                ERROR_EXIT(TM::ABORT_FLOW);
        }

        for(int i=0; i<data.size(); i++) {
                data[i]=((data[i]==1) ? receiveHiPhysIdx : receiveLoPhysIdx);
        }
}

std::vector<int> splitOverlayData(const uint64_t& data, const int& dataSize, const int& wordSize) {

        //dataSize is the total number of bits in the overlay data passed in
        //wordSize is the total number of bits in each word, # of pins in overlay pingroup
        ...
        //if not an even multiple, probably should error out, just issue warning for now
        if(dataSize%wordSize != 0) {
                cerr << "Warning, dataSize should be an even multiple of wordSize" << endl;
                //ERROR_EXIT(TM::ABORT_FLOW);
        }
        else {
                mult=dataSize/wordSize;
                //create bit mask, NOTE we are assuming wordSize is <= 32 bits
                uint64_t mask = (1ULL << wordSize) - 1;
                for(int i=mult-1; i>=0; i--) {
                        int thisWord = ((data&(mask<<(i*wordSize)))>>i*wordSize);
                        dataArray.push_back(thisWord);
                        //cout << "data word " << i << " = " << hex << thisWord << dec << endl;
                }
        }
...
void reverseBits(bitset <64>& ovbits, const int& bits) {
        bitset <64> local_bits(ovbits);
        unsigned int j=bits-1;
        for(int i=0; i<bits; i++) {
                ovbits[i]=local_bits[j];
                j--;
        }
}

void overlayTask(const string& labelName, const string& pinGroup, map<int, uint64_t>& vecData, const int& dataWordSize, const int& startVec, const bool& rBits, const OVERLAY_TYPE& ovlType, const int& receiveLoPhysIdx, const int& receiveHiPhysIdx) {

		if(vecData.size()<1) {
			cerr << "Overlay data map is empty, exiting..."  << endl;
			ERROR_EXIT(TM::ABORT_FLOW);
		}

		STRING_VECTOR plist = PinUtility.getDigitalPinNamesFromPinList(pinGroup, TM::ALL_DIGITAL,true,true,PIN_UTILITY::DEFINITION_ORDER);

        std::vector<VEC_TRANSFER_TASK> vTask;

        FOR_EACH_SITE_BEGIN();

			int site;

			//in this case, just overlay site 1 data to all sites
			if((int)vecData.size()<CURRENT_SITE_NUMBER()) {
				site=1;
				cout << "Warning: overlaying site 1 data to site " << site << endl;
			}
			else
				site=CURRENT_SITE_NUMBER();

			cout << "Overlaying 0x" << hex << vecData[site] << dec << " on site " << site << endl;

			std::vector<int> vecDataArray = splitOverlayData(vecData[site], dataWordSize, plist.size());

			size_t bits=plist.size();

			int vecOffset=0;

			for(unsigned int i=0; i<vecDataArray.size(); i++) {

				bitset<64> vecbits(vecDataArray[i]);

				VEC_TRANSFER_TASK thisTask;

				thisTask.label(labelName).vecNumber(startVec+vecOffset).blockSize(1);

				if(rBits)
					reverseBits(vecbits, bits);

				for(unsigned int p=0; p<plist.size(); p++) {

					ARRAY_I pdata(1);
					pdata.init(vecbits[p]);

					if(ovlType==OVL_RECEIVE)
						getPhysWfIndexes(receiveLoPhysIdx, receiveHiPhysIdx, pdata);

					thisTask.pin(plist[p]).data(pdata);
				}

				vTask.push_back(thisTask);
        ...
        //download all overlay data
        for(unsigned int v=0; v<vTask.size(); v++) {
        	vTask[v].execute();
        }
...
void overlayTask(const string& labelName, const string& pinGroup, const uint64_t& vecData, const int& dataWordSize, const int& startVec, const bool& rBits, const OVERLAY_TYPE& ovlType, const int& receiveLoPhysIdx, const int& receiveHiPhysIdx) {

        STRING_VECTOR plist = PinUtility.getDigitalPinNamesFromPinList(pinGroup, TM::ALL_DIGITAL,true,true,PIN_UTILITY::DEFINITION_ORDER);

        //cout << "Overlaying 0x" << hex << vecData << dec << " on all sites " << endl;

        std::vector<int> vecDataArray = splitOverlayData(vecData, dataWordSize, plist.size());
        ...
        for(unsigned int i=0; i<vecDataArray.size(); i++) {
                ...
                VEC_TRANSFER_TASK vTask;

                vTask.label(labelName).vecNumber(startVec+vecOffset).blockSize(1);
