...
/// Returns the value of the test suiteName+testName parameter supplied from the test suite, or if not supplied falls back
/// to the test suite name
string Base::stdftestName() {
	if (_testName == "") {
		return suiteName;
	} else {
		return suiteName + "." + _testName;
	}
}
